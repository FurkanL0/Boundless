...
# Docker ile ilgili bağımlılıkları (docker.io, docker-compose) bu adımda da kaldırabiliriz,
# çünkü DinD template'i bunları zaten sağlamalı.
# Ancak, betiğin diğer kısımları Docker istemcisini veya compose'u kullanabilir,
# bu yüzden şimdilik burada bırakıyorum.
...
# NOT: Quickpod DinD template'i kullanıldığı için Docker kurulum adımı atlanıyor.
# Quickpod'un zaten bir Docker daemon'u sağlaması beklenir.
log "${YELLOW}[3/12] Quickpod DinD template'i kullanıldığı için Docker kurulum adımı atlanıyor.${RESET}"
log "${YELLOW}Docker servisi zaten Quickpod tarafından yönetiliyor olmalıdır.${RESET}"
# Eğer Quickpod DinD template'i içinde bir 'docker' kullanıcısı yoksa ve bu komut hata veriyorsa
# veya gereksizse aşağıdaki satırı yorum satırı yapabilirsiniz.
# root kullanıcısı için bu gerekli değildir.
if [ "$(id -un)" != "root" ]; then
    CURRENT_USER=$(id -un)
...
EOF
    ...
    log "${YELLOW}DinD ortamında Docker servisini yeniden başlatma atlanıyor. Değişikliklerin etkili olması için konteynerin yeniden başlatılması gerekebilir.${RESET}"
